...
const ApprovedRegistrations = () => {
  ...
  useEffect(() => {
    ...
    if (adminLoggedIn) {
      // Get approved registrations from localStorage
      const storedRegistrations = JSON.parse(localStorage.getItem('registrations') || '[]');
      const approvedRegs = storedRegistrations.filter((reg: RegistrationType) => reg.status === 'Diterima');
      setRegistrations(approvedRegs);
      setFilteredRegistrations(approvedRegs);
    }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1">
        <div className="p-8">
          <h1 className="text-3xl font-bold text-school-primary mb-8">Pendaftar yang Diterima</h1>
          
          <div className="bg-white p-6 rounded-lg shadow mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-school-primary mb-4 md:mb-0">Daftar Pendaftar yang Diterima</h2>
            ...
            {filteredRegistrations.length > 0 ? (
              <Table>
                <TableCaption>Daftar pendaftar yang telah diterima</TableCaption>
                <TableHeader>
                  <TableRow>
              ...
              <div className="text-center py-8 text-gray-500">
                {searchTerm ? 'Tidak ada hasil yang sesuai dengan pencarian' : 'Belum ada pendaftar yang diterima'}
              </div>
            )}
          </div>
        </div>
