...
import { Clock, Search, Eye, Check, X } from 'lucide-react';
...
const PendingRegistrations = () => {
  ...
  useEffect(() => {
    ...
    if (adminLoggedIn) {
      ...
      const pendingRegs = storedRegistrations.filter((reg: RegistrationType) => reg.status === 'Menunggu Verifikasi');
      setRegistrations(pendingRegs);
      setFilteredRegistrations(pendingRegs);
    }
  ...
  const rejectRegistration = (id: number) => {
    const allRegistrations = JSON.parse(localStorage.getItem('registrations') || '[]');
    const updatedRegistrations = allRegistrations.map((reg: RegistrationType) => 
      reg.id === id ? { ...reg, status: 'Ditolak' } : reg
    );
    
    localStorage.setItem('registrations', JSON.stringify(updatedRegistrations));
    
    // Update local state by removing the rejected registration
    const newFilteredList = filteredRegistrations.filter(reg => reg.id !== id);
    setFilteredRegistrations(newFilteredList);
    ...
    toast.success('Status pendaftaran berhasil diubah menjadi Ditolak');
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1">
        <div className="p-8">
          <h1 className="text-3xl font-bold text-school-primary mb-8">Pendaftar yang Menunggu Verifikasi</h1>
          
          <div className="bg-white p-6 rounded-lg shadow mb-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-school-primary mb-4 md:mb-0">Daftar Pendaftar yang Perlu Diverifikasi</h2>
            ...
            {filteredRegistrations.length > 0 ? (
              <Table>
                <TableCaption>Daftar pendaftar yang menunggu verifikasi</TableCaption>
                <TableHeader>
                  <TableRow>
              ...
              <div className="text-center py-8 text-gray-500">
                {searchTerm ? 'Tidak ada hasil yang sesuai dengan pencarian' : 'Tidak ada pendaftar yang menunggu verifikasi'}
              </div>
            )}
          </div>
        </div>
